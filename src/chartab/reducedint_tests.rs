...
fn test_linalgreducedint_identities() {
    ...
    // Inverse
    assert_eq!(one / i5_7, i5_7.inv().unwrap());
    assert_eq!(i1_7 / i3_7, i3_7.inv().unwrap());
    assert_eq!(i4_7.inv().unwrap(), i2_7);
    assert_eq!(one.inv().unwrap(), one);
    assert!(zero.inv().is_none());
    assert!(i0_7.inv().is_none());
}
...
fn test_linalgreducedint_array() {
    ...
    let arr_6 = arr * i2_5.inv().unwrap().residue();
