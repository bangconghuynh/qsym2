...
#[test]
fn test_drivers_real_space_function_analysis_if7() {
    ...
    // E1'' (xz^3)
    // ~~~~~~~~~~~
    let e1dd_pes = RealSpaceFunction::<f64, _>::builder()
        .function(|pt| pt.x * pt.z.powi(3))
        .grid_points(grid_points.clone())
        .build()
        .unwrap();

    let pes_params = RealSpaceFunctionRepAnalysisParams::<f64>::builder()
    ...
    let mut pes_driver =
        RealSpaceFunctionRepAnalysisDriver::<UnitaryRepresentedSymmetryGroup, f64, _>::builder()
            .parameters(&pes_params)
            .angular_function_parameters(&afa_params)
            .real_space_function(&e1dd_pes)
            .weight(&weight)
    ...
    // E2'' (xyz)
    // ~~~~~~~~~~
    let e2dd_pes = RealSpaceFunction::<f64, _>::builder()
    ...
    // A2'' + E1' (z + yz^2)
    // ~~~~~~~~~~~~~~~~~~~~~
    let a2dde1d_pes = RealSpaceFunction::<f64, _>::builder()
    ...
    let mut pes_driver =
        RealSpaceFunctionRepAnalysisDriver::<UnitaryRepresentedSymmetryGroup, f64, _>::builder()
            .parameters(&pes_params)
            .angular_function_parameters(&afa_params)
            .real_space_function(&a2dde1d_pes)
            .weight(&weight)
            .symmetry_group(pd_res)
            .build()
            .unwrap();
