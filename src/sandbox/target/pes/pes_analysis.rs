...
where
    ...
    PES<Complex<T>>: SymmetryTransformable + Overlap<Complex<T>, Ix1>,
{
...
impl<'a, G, T> Orbit<G, PES<T>> for PESSymmetryOrbit<'a, G, T>
where
    G: SymmetryGroupProperties + Clone,
    T: ComplexFloat + fmt::Debug + Lapack,
    PES<T>: SymmetryTransformable,
{
    type OrbitIter = OrbitIterator<'a, G, PES<T>>;

    fn group(&self) -> &'a G {
        self.group
    }

    fn origin(&self) -> &PES<T> {
        self.origin
    }
...
impl<'a, G, T> RepAnalysis<G, PES<T>, T, Ix1> for PESSymmetryOrbit<'a, G, T>
where
    G: SymmetryGroupProperties + Clone,
