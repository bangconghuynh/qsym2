...
#[test]
fn test_real_space_function_orbit_rep_analysis_d4h() {
    // env_logger::init();
    ...
    // A1g
    let a1g_real_space_function = RealSpaceFunction::builder()
        .function(|pt| (pt - Point3::<f64>::origin()).magnitude_squared())
        .grid_points(grid_points.clone())
        .build()
        .unwrap();

    let mut orbit_a1g_real_space_function = RealSpaceFunctionSymmetryOrbit::builder()
        .origin(&a1g_real_space_function)
        .group(&group_u_d4h)
        ...
        .build()
        .unwrap();
    let _ = orbit_a1g_real_space_function
        .calc_smat(Some(&weight), None, true)
        .unwrap()
        .calc_xmat(false);
    assert_eq!(
        orbit_a1g_real_space_function.analyse_rep().unwrap(),
        DecomposedSymbol::<MullikenIrrepSymbol>::new("||A|_(1g)|").unwrap()
    );

    // B1g (x2 - y2)
    let b1g_real_space_function = RealSpaceFunction::builder()
        .function(|pt| pt.x.powi(2) - pt.y.powi(2))
        .grid_points(grid_points.clone())
        .build()
        .unwrap();

    let mut orbit_b1g_real_space_function = RealSpaceFunctionSymmetryOrbit::builder()
        .origin(&b1g_real_space_function)
        .group(&group_u_d4h)
        .integrality_threshold(1e-7)
        .linear_independence_threshold(1e-7)
        .symmetry_transformation_kind(SymmetryTransformationKind::Spatial)
        ...
        .build()
        .unwrap();
    let _ = orbit_b1g_real_space_function
        .calc_smat(Some(&weight), None, true)
        .unwrap()
        .calc_xmat(false);
    assert_eq!(
        orbit_b1g_real_space_function.analyse_rep().unwrap(),
        DecomposedSymbol::<MullikenIrrepSymbol>::new("||B|_(1g)|").unwrap()
    );

    // B2g (xy)
    let b2g_real_space_function = RealSpaceFunction::builder()
        .function(|pt| pt.x * pt.y)
        .grid_points(grid_points.clone())
        .build()
        .unwrap();

    let mut orbit_b2g_real_space_function = RealSpaceFunctionSymmetryOrbit::builder()
        .origin(&b2g_real_space_function)
        .group(&group_u_d4h)
        .integrality_threshold(1e-7)
        .linear_independence_threshold(1e-7)
        .symmetry_transformation_kind(SymmetryTransformationKind::Spatial)
        ...
        .build()
        .unwrap();
    let _ = orbit_b2g_real_space_function
        .calc_smat(Some(&weight), None, true)
        .unwrap()
        .calc_xmat(false);
    assert_eq!(
        orbit_b2g_real_space_function.analyse_rep().unwrap(),
        DecomposedSymbol::<MullikenIrrepSymbol>::new("||B|_(2g)|").unwrap()
    );

    // Eu (y)
    let eu_real_space_function = RealSpaceFunction::builder()
        .function(|pt| 1.5 * pt.y)
        .grid_points(grid_points)
        .build()
        .unwrap();

    let mut orbit_eu_real_space_function = RealSpaceFunctionSymmetryOrbit::builder()
        .origin(&eu_real_space_function)
        .group(&group_u_d4h)
        .integrality_threshold(1e-7)
        .linear_independence_threshold(1e-7)
        .symmetry_transformation_kind(SymmetryTransformationKind::Spatial)
        ...
        .build()
        .unwrap();
    let _ = orbit_eu_real_space_function
        .calc_smat(Some(&weight), None, true)
        .unwrap()
        .calc_xmat(false);
    assert_eq!(
        orbit_eu_real_space_function.analyse_rep().unwrap(),
    ...
    // B1u (xyz)
    let b1u_real_space_function = RealSpaceFunction::builder()
        .function(|pt| pt.x * pt.y * pt.z)
        .grid_points(grid_points.clone())
        .build()
        .unwrap();

    let mut orbit_b1u_real_space_function = RealSpaceFunctionSymmetryOrbit::builder()
        .origin(&b1u_real_space_function)
        .group(&group_u_d4h)
        .integrality_threshold(1e-7)
        .linear_independence_threshold(1e-7)
        .symmetry_transformation_kind(SymmetryTransformationKind::Spatial)
        ...
        .build()
        .unwrap();
    let _ = orbit_b1u_real_space_function
        .calc_smat(Some(&weight), None, true)
        .unwrap()
        .calc_xmat(false);
    assert_eq!(
        orbit_b1u_real_space_function.analyse_rep().unwrap(),
        DecomposedSymbol::<MullikenIrrepSymbol>::new("||B|_(1u)|").unwrap()
    );

    let eg_real_space_function = RealSpaceFunction::builder()
        .function(|pt| pt.x * pt.z)
        .grid_points(grid_points)
        .build()
        .unwrap();

    let mut orbit_eg_real_space_function = RealSpaceFunctionSymmetryOrbit::builder()
        .origin(&eg_real_space_function)
        .group(&group_u_d4h)
        .integrality_threshold(1e-7)
        .linear_independence_threshold(1e-7)
        .symmetry_transformation_kind(SymmetryTransformationKind::Spatial)
        ...
        .build()
        .unwrap();
    let _ = orbit_eg_real_space_function
        .calc_smat(Some(&weight), None, true)
        .unwrap()
        .calc_xmat(false);
    assert_eq!(
        orbit_eg_real_space_function.analyse_rep().unwrap(),
