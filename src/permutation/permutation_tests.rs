...
fn test_permutation_cycles() {
    ...
    let perms = (0..rank)
        ...
        .map(|image| Permutation::<u8>::from_image(image).unwrap());
    for perm in perms {
        assert_eq!(
            perm,
            Permutation::<u8>::from_cycles(&perm.cycles()).unwrap()
        );
    }
}
...
fn test_permutation_composition() {
    ...
    let p_04213 = Permutation::<u8>::from_image(vec![0, 4, 2, 1, 3]).unwrap();
    let p_34120 = Permutation::<u8>::from_image(vec![3, 4, 1, 2, 0]).unwrap();
    let p_12403 = Permutation::<u8>::from_image(vec![1, 2, 4, 0, 3]).unwrap();
    assert_eq!(p_34120, &p_32104 * &p_04213);
    assert_eq!(p_12403, &p_04213 * &p_32104);

    let p_12340 = Permutation::<u8>::from_image(vec![1, 2, 3, 4, 0]).unwrap();
    let p_23401 = Permutation::<u8>::from_image(vec![2, 3, 4, 0, 1]).unwrap();
...
fn test_permutation_lehmer() {
    let p201 = Permutation::<u8>::from_image(vec![2, 0, 1]).unwrap();
    assert_eq!(&p201.lehmer(None), &[2, 0, 0]);

    let p1506432 = Permutation::<u8>::from_image(vec![1, 5, 0, 6, 3, 4, 2]).unwrap();
    assert_eq!(&p1506432.lehmer(None), &[1, 4, 0, 3, 1, 1, 0]);

    let p2154603 = Permutation::<u8>::from_image(vec![2, 1, 5, 4, 6, 0, 3]).unwrap();
    assert_eq!(&p2154603.lehmer(None), &[2, 1, 3, 2, 2, 0, 0]);
    assert_eq!(p2154603.lehmer_index(None), 1648);
    assert_eq!(
        Permutation::<u8>::from_lehmer_index(1648, 7).unwrap(),
        p2154603
    );
    ...
    let p24107635 = Permutation::<u8>::from_image(vec![2, 4, 1, 0, 7, 6, 3, 5]).unwrap();
    assert_eq!(&p24107635.lehmer(None), &[2, 3, 1, 0, 3, 2, 0, 0]);

    let p24107635d = Permutation::<u8>::from_lehmer(p24107635.lehmer(None)).unwrap();
