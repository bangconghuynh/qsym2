...
* either an *orbit basis* generated by the action of a group $\mathcal{G}$ on a set of *origin Slater determinants* $\{ \Psi^x_{\mathrm{SD}} \}$:

    $$
        \mathcal{B}_{\mathcal{G} \cdot \{ \Psi^x_{\mathrm{SD}} \}} = \mathcal{G} \cdot \{ \Psi^x_{\mathrm{SD}} \},
    $$

    where $\lvert \{ \Psi^x_{\mathrm{SD}} \} \rvert$ is typically smaller compared to the group order $\lvert \mathcal{G} \rvert$, and the cardinality of $\mathcal{B}_{\mathcal{G} \cdot \{ \Psi^x_{\mathrm{SD}} \}}$ is given by $\lvert \mathcal{G} \rvert \times \lvert \{ \Psi^x_{\mathrm{SD}} \} \rvert$,
