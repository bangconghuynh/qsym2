...
=== "Python"
    ...
    === "Eager basis"
        ...
        19. :fontawesome-solid-users: This specifies the Slater determinants in the basis for non-orthogonal configuration interaction (NOCI).
        20. :fontawesome-solid-users: The non-orthogonal configuration interaction (NOCI) energies supplied as a one-dimensional `numpy` array.
        21. :fontawesome-solid-users: The non-orthogonal configuration interaction (NOCI) coefficients supplied as a two-dimensional `numpy` array where each column contains the coefficients for one NOCI solution.
